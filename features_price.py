...
from common                         import past_and_future_periods, moving_average_periods, close_column_name
from common                         import determine_column_name_close_past, determine_column_name_close_future, determine_column_name_close_moving_average
...
def time_shifted_close( share_data ):   
    for period_no in past_and_future_periods:       
        new_past_close_column    = determine_column_name_close_past  ( period_no )
        new_future_close_column  = determine_column_name_close_future( period_no )

        share_data[ new_past_close_column ]       = share_data[ close_column_name ].shift( period_no )
        share_data[ new_future_close_column ]     = share_data[ close_column_name ].shift( period_no * -1 )

        share_data[ new_past_close_column   ].fillna  ( share_data[ close_column_name ], inplace=True)
        share_data[ new_future_close_column ].fillna( share_data[ close_column_name ], inplace=True)
    return ( share_data )

def close_moving_average( share_data ):   
    for period_no in moving_average_periods:
        new_moving_average_col = determine_column_name_close_moving_average( period_no )

        share_data[ new_moving_average_col ] = share_data.close.rolling(period_no).mean() 

        share_data[ new_moving_average_col ].fillna  ( share_data[ close_column_name ], inplace=True)
    return ( share_data )
