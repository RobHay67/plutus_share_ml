...
from common                         import col_name_moving_average, col_name_volume_past, col_name_volume_future, past_and_future_periods, moving_average_periods
...
minutes_per_day = 360
average_volume_per_minute_colume_name = 'volume_average_per_minute'
...
def avg_vol_per_minute( share_df ):
    if volume_colume_name in share_df.columns:
        function_start_time = time.time()
        log_process_commencing( str( 'adding average volume to OHLC df' )  )

        share_df[average_volume_per_minute_colume_name]    = share_df[ volume_colume_name ] / minutes_per_day               
...
def time_shifted_average_volume( share_data ):
    for period_no in past_and_future_periods:
        ...
        share_data[ new_past_vol_col ]       = share_data[ average_volume_per_minute_colume_name ].shift( period_no )
        share_data[ new_future_vol_col ]     = share_data[ average_volume_per_minute_colume_name ].shift( period_no * -1 )

        share_data[ new_past_vol_col ].fillna  ( share_data[ average_volume_per_minute_colume_name ], inplace=True)
        share_data[ new_future_vol_col ].fillna( share_data[ average_volume_per_minute_colume_name ], inplace=True)

    return ( share_data )

def volume_moving_average( share_data ):  
    for period_no in moving_average_periods:
        new_moving_average_col, new_moving_average_per_minute_col  = col_name_moving_average( period_no )
        ...
        share_data[ new_moving_average_col            ].fillna  ( share_data[ volume_colume_name ],                    inplace=True)
        share_data[ new_moving_average_per_minute_col ].fillna  ( share_data[ average_volume_per_minute_colume_name ], inplace=True)
    return ( share_data )
